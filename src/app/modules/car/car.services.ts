...
const updateCar = async (_id: string, updatedData: Partial<TCar>) => {
    const result = await Car.updateOne({ _id: new Types.ObjectId(_id) }, { $set: updatedData })
...
const deleteCar = async (_id: string) => {
    const result = await Car.updateOne({ _id: new Types.ObjectId(_id) }, { $set: { isDeleted: true } })
    return result
}
